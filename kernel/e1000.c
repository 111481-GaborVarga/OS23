...
e1000_recv(void)
{
  ...
  // Create and deliver a mbuf for each packet (using net_rx()).
  ...
  while ((rx_ring[cur].status & E1000_RXD_STAT_DD) != 0) {
    ...
    */
    ...
    // Then allocate a new mbuf (because net_rx() maybe hanging on to the mbuf passed to it)
    // and program its head pointer into the descriptor. Clear the descriptor’s status bits to zero.
